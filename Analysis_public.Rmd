...
Sample annotations
```{r}
data <- read.csv("sample_description.csv", stringsAsFactors=F)
...
```{r}
Y.IR <- read.csv('mutations_ir_samples.csv', row.names = 1)
data.IR <- data[rownames(Y.IR),]
```

Total gamma-radiation induced mutation counts across samples in different genotypes:
```{r}
data.IR$total_muts <- rowSums(Y.IR)
df <- data.IR[data.IR$Mutagen == 'Radiation',]
ggplot(data = df, aes(x = Sample, y = total_muts, fill = Genotype)) + geom_bar(stat = 'identity') + facet_grid(Sample ~ Genotype) + guides(fill = F)
```
...
data.IR <- data[which(data$Mutagen %in% c('Radiation','Xray') | (is.na(data$Mutagen) & data$Genotype %in% genotypes)),]
CD2Mutant.IR <- CD2Mutant[which(data$Mutagen %in% c('Radiation','Xray') | (is.na(data$Mutagen) & data$Genotype %in% genotypes))]
```
...
interactions <- sapply(which(!is.na(data.IR$Mutagen)), function(j) {
  ...
  return(paste(data.IR$Genotype[j],m,sep=":"))
})
...
```{r}
X <- data.frame(sapply(unique(data.IR$Genotype), function(x) as.numeric(data.IR$Genotype == x)),
...
allclust <- read.csv('clustered_mutations.csv')
...
# assign correct names for plotting
